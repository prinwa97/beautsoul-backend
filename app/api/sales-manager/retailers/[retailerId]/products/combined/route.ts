...
export const GET = apiHandler(async function GET(
  ...
  const cleanRetailerId = String(retailerId || "").trim();
  ...
  const purchasedMap = new Map<
    string,
    { productName: string; purchasedQty: number; lastPurchaseAt: Date | null }
  >();
  ...
  for (const o of orders) {
    ...
    for (const it of o.items || []) {
      ...
      const rec = purchasedMap.get(key) || {
        productName: pn,
        purchasedQty: 0,
        lastPurchaseAt: null,
      };
      ...
      if (ca && (!rec.lastPurchaseAt || ca.getTime() > rec.lastPurchaseAt.getTime())) {
        rec.lastPurchaseAt = ca;
      }
      rec.productName = pn;
  ...
  if (stockFound) {
    ...
    const stockRows = await Stock.findMany({
      where: { ownerType: "RETAILER", ownerId: cleanRetailerId },
  ...
  if (!audit) {
    audit = await prisma.retailerStockAudit.findFirst({
      where: { retailerId: cleanRetailerId },
      orderBy: { createdAt: "desc" },
      select: { id: true, createdAt: true },
    });
  }
  ...
  const rows = Array.from(keys).map((k) => {
    ...
    const currentStock = stockUnknown
      ? 0
      : Math.max(0, num(hasAudit ? physicalAudit : pendingSystem));
    ...
    const anomaly =
      !stockUnknown && purchasedQty > 0 && purchasedQty - currentStock < 0;

    const displayName =
      p?.productName ||
      physicalNameMap.get(k) ||
      pendingNameMap.get(k) ||
      "—";
    ...
    if (stockUnknown) {
      ...
      const reorderThreshold = Math.max(2, Math.ceil(purchasedQty * 0.2));
  ...
  rows.sort(
    (a, b) =>
      num(b.soldQty ?? -1) - num(a.soldQty ?? -1) ||
      num(b.purchasedQty) - num(a.purchasedQty)
  );
