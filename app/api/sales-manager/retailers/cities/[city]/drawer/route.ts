...
export const GET = apiHandler(async function GET(
  ...
  const byProduct = new Map<
    string,
    { productName: string; sales: number; ordersSet: Set<string> }
  >();
  ...
  const cityPlan = [
    {
      title: "Reactivate inactive retailers",
      targets: Math.min(5, Math.max(0, retailerCount - byRetailer.size)),
    },
    {
      title: "Upsell top 2 products in top retailers",
      targets: Math.min(5, topRetailers.length),
    },
    {
      title: "Revive 1 slow mover SKU with bundling",
      targets: 1,
    },
  ];
