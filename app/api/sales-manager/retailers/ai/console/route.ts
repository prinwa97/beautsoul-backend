...
// IST start-of-day -> returned in UTC Date
...
function startOfFYUTC(now = new Date()) {
  ...
  return new Date(Date.UTC(fyStartYear, 3, 1, 0, 0, 0));
}
...
function getRanges(url: URL) {
  ...
  if (mode === "TODAY") {
    ...
    const t = parseYMD(url.searchParams.get("to"));
    if (!f || !t) {
      throw badRequest("CUSTOM requires from & to");
    }
    rangeFrom = f;
    rangeTo = new Date(t.getTime() + 24 * 60 * 60 * 1000);
  } else if (mode === "MONTH") {
  ...
  return {
    mode,
    day: startOfDayIST(new Date()),
    current: { from: rangeFrom, to: rangeTo },
    previous: { from: prevFrom, to: prevTo },
    engine: { from: engineFrom, to: engineTo, prevFrom: enginePrevFrom, prevTo: enginePrevTo, days: engineDays },
  };
}
...
async function resolveSalesManagerAuth(req: Request, debugAuth: boolean) {
  ...
  if (!userId) {
    const err = unauthorized("UNAUTHORIZED");
    if (debugAuth) {
      (err as any).details = {
        message: "No session user found (cookie missing) and no x-user-id header provided.",
        debug: {
          session: session ? { id: sessionId, role: sessionRole } : null,
          headerUserId,
          headerUserRole,
        },
      };
  ...
  if (!sm) {
    const err = unauthorized("SESSION_USER_NOT_IN_DB");
    (err as any).details = { salesManagerId: userId };
    throw err;
  }
  ...
  if (dbRole !== "SALES_MANAGER" && dbRole !== "ADMIN") {
    const err = forbidden("FORBIDDEN");
    (err as any).details = { message: `DB role not allowed: ${dbRole}` };
    throw err;
  }
  ...
  return {
    ...
    role: dbRole,
    session,
    debug: { sessionRole, headerUserRole, userId: sm.id },
  };
}
...
function proofStrengthFrom(confidence: number) {
...
async function getAnalyticsPack(args: {
  rangeFrom: Date;
  rangeTo: Date;
  prevFrom: Date;
  prevTo: Date;
  ...
  const { rangeFrom, rangeTo, prevFrom, prevTo, distId, city, limit } = args;
  ...
  return {
    ...
    slowMoversByCity,
    summary: {
      topCityName: topCities[0]?.city || null,
...
async function getRiskRetailers(distId: string, city: string) {
  ...
  const oldOrderIds = lastOrders
    ...
    .filter((x) => x.lastAt && daysBetween(now, x.lastAt) >= 30)
    .slice(0, 40)
    .map((x) => x.retailerId);
  ...
  for (const r of zeroOrderRetailers) {
    if (!seen.has(r.id)) {
      seen.add(r.id);
      riskRetailersMerged.push({
        id: r.id,
        name: cleanStr((r as any).name) || r.id,
        city: cleanStr((r as any).city) || "—",
      });
    }
  }
  ...
  return riskRetailersMerged;
}

/* -------------------------------------------------------------------------- */
/*                        latest order info safer lookup                       */
/* -------------------------------------------------------------------------- */

async function getLastOrderInfo(retailerIds: string[]) {
  const out = new Map<string, { at: string | null; amount: number | null }>();
  if (!retailerIds.length) return out;
  ...
  const rows = await prisma.order.findMany({
    ...
    orderBy: [{ retailerId: "asc" }, { createdAt: "desc" }],
    take: 20000,
  });
...
async function runLevel4Engine(args: {
  distId: string;
  city: string;
  topRetailers: Array<{ retailerId: string }>;
  riskRetailersMerged: Array<{ id: string }>;
  engineFrom: Date;
  engineTo: Date;
  enginePrevFrom: Date;
  enginePrevTo: Date;
}) {
  const { distId, city, topRetailers, riskRetailersMerged, engineFrom, engineTo, enginePrevFrom, enginePrevTo } = args;
  ...
  for (const rid of engineRetailerIds) {
    ...
    const retailerType = decideRetailerType({
      recencyDays,
    ...
    for (const key of candidateKeys) {
      ...
      const proofStrength = proofStrengthFrom(conf);
...
/* -------------------------------------------------------------------------- */
/*                          task plan + micro targets                         */
/* -------------------------------------------------------------------------- */

function toTaskTargets(
  list: EngineTarget[],
  why: string,
  retailerIndex: Map<string, { id: string; name: string; city: string; distributorName: string }>,
  lastOrderInfo: Map<string, { at: string | null; amount: number | null }>
): TaskTarget[] {
  return list.map((t) => {
...
async function seedAndReadTodayPlan(args: {
  salesManagerId: string;
  day: Date;
  distId: string;
  topCityName: string | null;
  topProducts: Array<{ productName: string }>;
  slowMoversByCity: Array<{ city: string; productName: string }>;
  riskRetailersMerged: Array<{ id: string }>;
  level4Targets: EngineTarget[];
  retailerIndex: Map<string, { id: string; name: string; city: string; distributorName: string }>;
  lastOrderInfo: Map<string, { at: string | null; amount: number | null }>;
}) {
  const {
  ...
  taskTargetsMap.set(
    ...
    toTaskTargets(
      reactivationTargets,
      "Inactive / high gap. Restart with 1–2 easy-win fast movers (only if stock available).",
      retailerIndex,
      lastOrderInfo
    )
  );
  taskTargetsMap.set(
    ...
    toTaskTargets(
      upsellTargets,
      "High potential retailer. Push top 2–4 products: gaps + city demand + basket affinity (stock-checked).",
      retailerIndex,
      lastOrderInfo
    )
  );
  taskTargetsMap.set(
    ...
    toTaskTargets(
      slowMoverTargets,
      "Revive slow movers using blocker questions + bundling with fast movers (affinity-based, stock-checked).",
      retailerIndex,
      lastOrderInfo
    )
  );
  ...
  const todayPlan = (todayPlanRaw || [])
    ...
    .map((t: any) => {
      ...
      const microTasks = targets.slice(0, 12).map((target, idx) => ({
        id: `${String(t.id)}__${idx + 1}`,
        priority: idx + 1,
        ...
        lastOrderAmount: target.lastOrderAmount,
        actionMode:
          typeKey === "REACTIVATE_RETAILER"
            ? "VISIT"
            : typeKey === "UPSELL_PRODUCTS"
            ...
            : "VISIT",
        proofStrength:
          target.recommendedProducts?.[0]?.confidence != null
            ? target.recommendedProducts[0].confidence >= 80
              ? "HIGH"
              : target.recommendedProducts[0].confidence >= 60
              ? "MEDIUM"
              : "LOW"
            : "MEDIUM",
        expectedImpactMin: (target.recommendedProducts || []).reduce((s, x) => s + num(x.expectedImpactMin), 0),
        expectedImpactMax: (target.recommendedProducts || []).reduce((s, x) => s + num(x.expectedImpactMax), 0),
        reason: target.personalizedReason,
        products: (target.recommendedProducts || []).map((x) => ({
          productName: x.productName,
          score: x.score,
...
function buildPerformance(args: {
  ...
  doneTasks: number;
  openCount: number;
  topCities: Array<{ city: string; orders: number; sales: number; growthPct: number }>;
  topProducts: Array<{ productName: string; orders: number; qty: number; sales: number; repeat: number; growthPct: number }>;
  riskRetailersMerged: Array<{ id: string; name: string; city: string }>;
  inventoryEnabled: boolean;
  level4Targets: EngineTarget[];
}) {
  const { totalTasks, doneTasks, openCount, topCities, topProducts, riskRetailersMerged, inventoryEnabled, level4Targets } = args;

  const topCity = topCities[0] || null;
  const topProduct = topProducts[0] || null;

  const completionRatio = totalTasks ? doneTasks / totalTasks : 0;
  const taskScore = completionRatio * 40;
  const growthScore = topCity ? clamp((topCity.growthPct + 20) * 0.4, 0, 20) : 0;
  const productScore = topProduct ? clamp((topProduct.growthPct + 20) * 0.25, 0, 15) : 0;
  const riskPenalty = clamp(riskRetailersMerged.length * 2, 0, 20);
  const inventoryScore = inventoryEnabled ? 15 : 5;
  const engineScore = clamp(level4Targets.length * 0.7, 0, 10);

  const score = clamp(Math.round(taskScore + growthScore + productScore + inventoryScore + engineScore - riskPenalty), 0, 100);

  const reasons: string[] = [];
  reasons.push(`Today Plan: ${openCount} tasks pending out of ${totalTasks}.`);
  ...
  if (topCity) {
    ...
    reasons.push(`Top City: — (no city sales in selected range).`);
  }
  ...
  if (topProduct) {
    ...
    reasons.push(`Top Product: — (no product sales in selected range).`);
  }

  if (riskRetailersMerged.length) {
    reasons.push(`Risk rule: retailers with (A) zero orders OR (B) last order >= 30 days ago. Found: ${riskRetailersMerged.length}.`);
  } else {
    reasons.push(`Risk rule: zero orders OR last order >= 30 days ago. Found: 0.`);
  }

  reasons.push(`Level-4: ${level4Targets.length} retailers analyzed using last 90 days orders.`);
  reasons.push(inventoryEnabled ? `Inventory: enabled — out-of-stock recommendations are hard-rejected.` : `Inventory: disabled — recommendations are sales-only.`);
...
function buildProof(args: {
  ...
  prevOrdersInRange: number;
  engineOrders90: number;
  enginePrevOrders90: number;
  level4Targets: EngineTarget[];
}) {
  const {
    mode,
  ...
  return {
    ...
    whyThisCity: topCity
      ? {
          city: topCity.city,
          orders: topCity.orders,
          sales: Math.round(topCity.sales),
          growthPct: Number(topCity.growthPct || 0),
          comparedTo: "previous selected period",
        }
      : null,
    whyThisProduct: topProduct
      ? {
          productName: topProduct.productName,
          orders: topProduct.orders,
          qty: topProduct.qty,
          sales: Math.round(topProduct.sales),
          repeatRetailers: topProduct.repeat,
          growthPct: Number(topProduct.growthPct || 0),
        }
      : null,
    whyRetailersAtRisk: riskRetailersMerged.slice(0, 10).map((r) => ({
      retailerId: r.id,
      retailerName: r.name,
      city: r.city,
      rule: "ZERO_ORDER_OR_LAST_ORDER_30_PLUS",
    })),
    whyRecommendations: level4Targets.slice(0, 12).map((t) => ({
      retailerId: t.retailerId,
      retailerName: t.retailerName,
      city: t.city,
      retailerType: t.dna.retailerType,
      recencyDays: t.dna.recencyDays,
      topRecommendation: t.recommendations[0]
        ? {
            productName: t.recommendations[0].productName,
            score: t.recommendations[0].score,
            confidence: t.recommendations[0].confidence,
            proofStrength: t.recommendations[0].proofStrength,
            reasons: t.recommendations[0].reasons,
          }
        : null,
    })),
    inventory: {
      enabled: inventoryEnabled,
      stockModelKey: stockModelKey || null,
      mode: inventoryEnabled ? "HARD_REJECT_OUT_OF_STOCK" : "SALES_ONLY",
    },
    engine: {
      version: "v2.1",
      days: engineDays,
      from: engineFrom.toISOString(),
      to: engineTo.toISOString(),
    },
    counts: {
      ordersInRange,
      prevOrdersInRange,
...
export const GET = apiHandler(async function GET(req: Request) {
  try {
    ...
    const auth = await resolveSalesManagerAuth(req, debugAuth);

    const salesManagerId = auth.salesManagerId;
    ...
    const ranges = getRanges(url);

    const analytics = await getAnalyticsPack({
      rangeFrom: ranges.current.from,
      rangeTo: ranges.current.to,
      prevFrom: ranges.previous.from,
      prevTo: ranges.previous.to,
      distId,
      city,
      limit,
    });

    const riskRetailersMerged = await getRiskRetailers(distId, city);

    const engine = await runLevel4Engine({
      distId,
      city,
      topRetailers: analytics.topRetailers,
      riskRetailersMerged,
      engineFrom: ranges.engine.from,
      engineTo: ranges.engine.to,
      enginePrevFrom: ranges.engine.prevFrom,
      enginePrevTo: ranges.engine.prevTo,
    });

    const lastOrderInfo = await getLastOrderInfo(engine.engineRetailerIds);

    const todayPack = await seedAndReadTodayPlan({
      salesManagerId,
      day: ranges.day,
      distId,
      topCityName: analytics.summary.topCityName,
      topProducts: analytics.topProducts,
      slowMoversByCity: analytics.slowMoversByCity,
      riskRetailersMerged,
      level4Targets: engine.level4Targets,
      retailerIndex: engine.retailerIndex,
      lastOrderInfo,
    });

    if (!todayPack.aiEnabled) {
      return NextResponse.json({ ok: true, aiEnabled: false, reason: "TASK_TABLE_MISSING" }, { status: 200 });
    }

    const totalTasks = todayPack.todayPlan.length;
    const doneTasks = todayPack.todayPlan.filter((t: any) => normalizeRole(t.status) === "DONE").length;
    ...
    const executiveBrief = [
      { id: "brief_today_plan", type: "TODAY_PLAN", title: `Today: ${openCount} tasks pending.`, count: openCount },
      {
        id: "brief_top_city",
        type: "TOP_CITY",
        title: `Best city today: ${analytics.summary.topCityName || "—"}`,
        count: analytics.topCities[0]?.sales || 0,
        city: analytics.summary.topCityName || "—",
      },
      {
        id: "brief_top_product",
        type: "TOP_PRODUCT",
        title: `Best-selling product: ${analytics.summary.topProductName || "—"}`,
        count: analytics.topProducts[0]?.sales || 0,
        productName: analytics.summary.topProductName || "—",
      },
      {
        id: "brief_risk",
        type: "RISK_ALERT",
        title: `Risk: ${riskRetailersMerged.length} retailers inactive / no orders`,
        count: riskRetailersMerged.length,
      },
      {
        id: "brief_level4",
        type: "AI_LEVEL4",
        title: `Level-4 Engine: ${engine.level4Targets.length} retailers analyzed (90d)`,
        count: engine.level4Targets.length,
      },
    ];
    ...
    if (riskRetailersMerged.length) {
      insights.push({
        ...
        evidence: riskRetailersMerged.slice(0, 5).map((r) => ({
          kind: "RETAILER",
          id: r.id,
          label: r.name,
          metric: r.city,
    ...
    const performance = buildPerformance({
      ...
      topProducts: analytics.topProducts,
      riskRetailersMerged,
      inventoryEnabled: engine.inventoryEnabled,
      level4Targets: engine.level4Targets,
    });
    ...
    const proof = buildProof({
      ...
      riskRetailersMerged,
      inventoryEnabled: engine.inventoryEnabled,
      stockModelKey: engine.stockModelKey,
      engineDays: ranges.engine.days,
      engineFrom: ranges.engine.from,
      engineTo: ranges.engine.to,
      ordersInRange: analytics.orders.length,
      prevOrdersInRange: analytics.prevOrders.length,
      engineOrders90: engine.o90.length,
      enginePrevOrders90: engine.o90prev.length,
      level4Targets: engine.level4Targets,
    });
    ...
    return NextResponse.json({
      ...
      engineRange: {
        from: ranges.engine.from.toISOString(),
        to: ranges.engine.to.toISOString(),
        days: ranges.engine.days,
      },
      filters: { distId: distId || null, city: city || null },
      inventory: {
        enabled: engine.inventoryEnabled,
        stockModelKey: engine.stockModelKey,
        note: engine.inventoryEnabled
          ? "Out-of-stock products are hard-rejected from recommendations."
          : "Stock model not found; recommendations are sales-only.",
      },
      executiveBrief,
      todayPlan: todayPack.todayPlan,
      leaderboards: {
        topRetailers: analytics.topRetailers.slice(0, limit),
        topCities: analytics.topCities,
        topProducts: analytics.topProducts,
        slowMoversByCity: analytics.slowMoversByCity,
      },
      level4: { targets: engine.level4Targets.slice(0, 40) },
      insights,
      performance,
      proof,
      debug: debugAuth
