...
export async function GET(req: Request, ctx: { params: Promise<{ productName: string }> }) {
  try {
    ...
    if (!prod) return json(false, { error: "INVALID_PRODUCT" }, 400);
    ...
    const byRetailer = new Map<string, { retailerId: string; retailerName: string; city: string; sales: number; ordersSet: Set<string> }>();
    const byCity = new Map<string, { city: string; sales: number; ordersSet: Set<string> }>();

    // co-occurrence bundle pairs (within same order)
    const co = new Map<string, number>(); // otherProduct -> sales amount in same orders

    // adoption gap helper: retailers who have orders in range but never bought this product
    const allRetailerSales = new Map<string, { retailerId: string; retailerName: string; city: string; sales: number }>();
    ...
    for (const o of orders) {
      ...
      const rr = allRetailerSales.get(rid) || { retailerId: rid, retailerName: rname, city: rcity, sales: 0 };
      rr.sales += orderTotal;
      ...
      const br = byRetailer.get(rid) || { retailerId: rid, retailerName: rname, city: rcity, sales: 0, ordersSet: new Set<string>() };
      br.sales += matched.reduce((s, it) => s + num(it.amount), 0);
      br.ordersSet.add(String(o.id));
      byRetailer.set(rid, br);

      const bc = byCity.get(rcity) || { city: rcity, sales: 0, ordersSet: new Set<string>() };
      bc.sales += matched.reduce((s, it) => s + num(it.amount), 0);
    ...
    for (const o of prevOrders) {
      for (const it of o.items || []) {
        if (cleanStr(it.productName) === prod) prevSales += num(it.amount);
      }
    }

    const topRetailers = Array.from(byRetailer.values())
      .map((x) => ({ retailerId: x.retailerId, retailerName: x.retailerName, city: x.city, sales: Math.round(x.sales), orders: x.ordersSet.size }))
      .sort((a, b) => b.sales - a.sales)
      .slice(0, 10);

    const topCities = Array.from(byCity.values())
      .map((x) => ({ city: x.city, sales: Math.round(x.sales), orders: x.ordersSet.size }))
      .filter((x) => x.city !== "—")
      .sort((a, b) => b.sales - a.sales)
      .slice(0, 10);

    // adoption gap: top overall retailers who did NOT buy this product
    ...
    // bundle pairs: top 8 co-occurring products
    const bundlePairs = Array.from(co.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 8)
      .map(([productName, coSales]) => ({
        productName,
        liftPct: 0, // optional later
