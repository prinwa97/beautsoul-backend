...
function buildScript(task: any) {
  ...
  const products = Array.isArray(task?.productNames) ? task.productNames.filter(Boolean) : [];
  ...
  if (type === "CITY_FOCUS") {
    return [
      ...
      products.length ? `Top products push: ${products.slice(0, 4).join(", ")}.` : "Top products push: fast movers + add-on SKUs.",
  ...
  return [
    ...
    products.length ? `Suggested add-ons: ${products.slice(0, 4).join(", ")}.` : "Suggested add-ons: Sunscreen + Facewash + Toothbrush (example).",
...
export async function GET(req: Request, ctx: { params: Promise<{ taskId: string }> }) {
  try {
    ...
    if (!id) return json(false, { error: "INVALID_TASK" }, 400);
    ...
    if (!task) return json(false, { error: "NOT_FOUND" }, 404);

    // Target retailers names (optional)
    const retailerIds = Array.isArray((task as any).retailerIds) ? (task as any).retailerIds : [];
    const retailers =
