...
function pillClass(kind: "ok" | "warn" | "bad") {
  if (kind === "bad") return "border-rose-200 bg-rose-100 text-rose-800";
  if (kind === "warn") return "border-amber-200 bg-amber-100 text-amber-800";
...
function riskFromRow(r: Row) {
  ...
  if (st === "DELIVERED" || st === "CANCELLED" || st === "REJECTED") {
    return { label: "Closed", kind: "ok" as const };
  }
...
export default function WarehouseOrdersPage() {
  ...
  const quickAlerts = useMemo(() => {
    ...
    if (stats.delay.critical24hPlus > 0) {
      a.push({
        text: `${stats.delay.critical24hPlus} orders SLA breach (24h+)`,
        kind: "bad",
      });
    }

    if (stats.delay.warn12hPlus > 0) {
      a.push({
        text: `${stats.delay.warn12hPlus} orders delayed (12h+)`,
        kind: "warn",
      });
    }

    if (stats.revenue.stuck12hPlusValue > 0) {
      a.push({
        text: `Revenue stuck (12h+): ${inr(stats.revenue.stuck12hPlusValue)}`,
        kind: "warn",
      });
    }

    if (!a.length) {
      a.push({ text: "No critical alerts ✅", kind: "ok" });
    }
    ...
    <div className="space-y-4 text-gray-900">
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <div className="text-2xl font-bold text-gray-900">Orders Control Tower</div>
            <div className="text-sm text-gray-600">
              Distributor Orders monitoring (schema-safe)
            </div>
          </div>
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center">
            <input
              ...
              className="w-full rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 outline-none placeholder:text-gray-400 focus:border-black md:w-80"
            />

            <select
              value={status}
              onChange={(e) => setStatus(e.target.value as any)}
              className="rounded-xl border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 outline-none focus:border-black"
            >
              {STATUS_OPTS.map((s) => (
            ...
            <button
              ...
              disabled={loading}
              type="button"
              className="rounded-xl bg-gradient-to-r from-pink-500 to-rose-500 px-4 py-2 text-sm font-semibold text-white shadow hover:opacity-95 disabled:cursor-not-allowed disabled:opacity-60"
            >
      ...
      <div className="grid grid-cols-1 gap-2 md:grid-cols-3">
        {quickAlerts.map((a, i) => (
          <div
            key={i}
            className={`rounded-2xl border px-3 py-2 text-sm font-medium ${pillClass(a.kind)}`}
          >
            {a.text}
          </div>
        ))}
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-5">
        ...
        <Card
          title="SLA 24h+"
          value={stats ? String(stats.delay.critical24hPlus) : "-"}
          sub="Critical"
          danger
        />
        <Card
          title="Delayed 12h+"
          value={stats ? String(stats.delay.warn12hPlus) : "-"}
          sub="Warning"
          warn
        />
        <Card
          title="Dispatched Today"
          value={stats ? String(stats.dispatchedToday) : "-"}
          sub="Heuristic"
        />
      </div>
          ...
          <div className="rounded-2xl border border-gray-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-2 text-sm text-gray-700">
              ...
              <span className="font-semibold text-gray-900">
                {inr(stats.revenue.pendingDispatchValue)}
              </span>
            </div>

            <div className="mt-1 text-sm text-gray-700">
              Stuck (12h+ & not dispatched):{" "}
              <span className="font-semibold text-gray-900">
                {inr(stats.revenue.stuck12hPlusValue)}
              </span>
            </div>

            <div className="mt-2 text-xs text-gray-500">
              Note: Order model me dispatch fields nahi hain, isliye stuck logic age + status se
          ...
          <table className="min-w-[1100px] w-full text-sm">
            <thead className="bg-pink-50 text-gray-700">
              <tr>
                ...
                <th className="px-3 py-3 text-left font-bold">Distributor</th>
                <th className="px-3 py-3 text-left font-bold">Retailer</th>
                ...
                <th className="px-3 py-3 text-left font-bold">Updated</th>
              </tr>
            </thead>
            ...
            <tbody className="text-gray-900">
              {rows.map((r) => {
                ...
                return (
                  <tr key={r.id} className="border-t border-gray-100 hover:bg-pink-50/40">
                    ...
                    <td className="px-3 py-3">
                      ...
                      <div className="text-xs text-gray-600">
                        {r.distributor?.city}, {r.distributor?.state}
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-3">
                      ...
                      <div className="text-xs text-gray-600">
                        {r.retailer?.city || "-"}, {r.retailer?.state || "-"}
                      </div>
                    </td>

                    <td className="px-3 py-3">
                      <span className="rounded-xl border border-gray-200 bg-white px-2 py-1 text-xs font-semibold text-gray-800">
                        {String(r.status)}
                      </span>
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <span
                        className={`rounded-xl border px-2 py-1 text-xs font-semibold ${pillClass(
                          risk.kind
                        )}`}
                      >
                        {risk.label}
                    ...
                    <td className="px-3 py-3 text-xs text-gray-700">{fmtDT(r.createdAt)}</td>
                    <td className="px-3 py-3 text-xs text-gray-700">{fmtDT(r.updatedAt)}</td>
                  </tr>
                );
              })}
              ...
              {!loading && !rows.length ? (
                <tr>
                  <td colSpan={9} className="px-3 py-8 text-center text-sm font-medium text-gray-600">
                    No orders found.
                  </td>
                </tr>
              ) : null}
